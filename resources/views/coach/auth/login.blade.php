...
<body class="hold-transition login-page auth-background"
    style="background-image:url('{{ url('/design/adminlte/dist/img/auth-background.png') }}')">
    <div class="login-box">
        <div class="login-logo">
            <div>
                <b>Captainy</b>
                </br>
                <span>Coaches Board</span>
            </div>
        </div>
        <!-- /.login-logo -->
        <div class="login-box-body">
            ...
            @if (session()->has('error'))
                @push('js')
                    <script>
                        toastr.error('{{ session('error') }}', 'Error Alert', {
                            timeOut: 10000,
                            closeButton: true,
                            progressBar: true
                        })
            ...
            @if (session()->has('success'))
                @push('js')
                    <script>
                        toastr.success('{{ session('success') }}', 'Success Alert', {
                            timeOut: 10000,
                            closeButton: true,
                            progressBar: true
                        })
            ...
            <a href="{{ coachUrl('forgot/password') }}" class="forget">Forgot my password</a><br>
            <a href="{{ coachUrl('register') }}" class="register">Create new account <i class="fa fa-arrow-right"></i> </a>
        </div>
        <!-- /.login-box-body -->
    </div>
    <!-- /.login-box -->
