<script>
    ...
    $(document).ready(function() {
        $(document).on('click', '.create-ajax', function() {});
    });
    ...
    $(document).ready(function() {
        ...
        $('#store_form').on('submit', function(event) {
            ...
            if ($('#imageUpload-create').prop('files')[0]) {
                formData.append('image', $('#imageUpload-create').prop('files')[0]);
            }
    ...
    $(document).ready(function() {
        $(document).on('click', '.show-ajax', function() {
            $.ajax({
                url: '{{ adminUrl('') }}/admins/' + $(this).data('ajax'),
                type: 'get',
                success: function(data) {
    ...
    $(document).ready(function() {
        $(document).on('click', '.edit-ajax', function() {
            ...
            $.ajax({
                url: '{{ adminUrl('') }}/admins/' + $(this).data('ajax') + '/edit',
                type: 'get',
                success: function(data) {
    ...
    $(document).ready(function() {
        $(document).on('submit', '#update_form', function(event) {
            ...
            if ($('#imageUpload-edit').prop('files')[0]) {
                formData.append('image', $('#imageUpload-edit').prop('files')[0]);
            }

            $.ajax({
                url: '{{ adminUrl('') }}/admins/' + $('#ajax_edit_content #id').val() +
                    '/update',
                method: "post",
                data: formData,
                contentType: false,
    ...
    function clearFields() {
        ...
        $('#ajax_create_content #imagePreview-create').css({
            'background-image': 'url({{ url('storage/images/admins/default_admin.png') }})'
        });
